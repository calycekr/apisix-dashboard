...
export const test = baseTest.extend<object, { workerStorageState: string }>({
  ...
  workerStorageState: [
    async ({ browser }, use) => {
      ...
      let settingsModal = page.getByRole('dialog');
      if ((await settingsModal.count()) === 0) {
        await page
          .getByRole('button', { name: 'Open settings' })
          .click({ force: true });
        settingsModal = page.getByRole('dialog');
      }
      await expect(settingsModal).toBeVisible();
      const adminKeyInput = settingsModal.getByPlaceholder(
        'Enter your APISIX Admin Key'
      );
      await adminKeyInput.clear();
      await adminKeyInput.fill(adminKey);
      await settingsModal.getByRole('button', { name: 'Test' }).click();
