  ...
  const selectOptions = useMemo(
    () =>
      (options ?? [])
        ...
        label: (
          <span>
            <Typography.Text code style={{ fontSize: 'var(--app-font-size-sm)' }}>{opt.id}</Typography.Text>
            {opt.name && (
              <Typography.Text type="secondary" style={{ marginLeft: 8, fontSize: 'var(--app-font-size-sm)' }}>
                {opt.name}
              </Typography.Text>
            )}
          </span>
