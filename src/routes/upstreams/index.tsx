...
function RouteComponent() {
        ...
        render: (_, record) => {
          ...
          return (
            <span style={{ display: 'inline-flex', flexWrap: 'wrap', gap: 3 }}>
              {visible.map((h) => (
                <Tag key={h} style={{ margin: 0, fontSize: 'var(--app-font-size-xs)', fontFamily: 'var(--app-font-monospace)' }}>{h}</Tag>
              ))}
              {remaining > 0 && (
                <Tooltip title={hosts.slice(2).join(', ')}>
                  <Tag style={{ margin: 0, fontSize: 'var(--app-font-size-xs)' }}>+{remaining}</Tag>
                </Tooltip>
              )}
            </span>
          );
  ...
  return (
    <>
      <PageHeader
        title="Upstreams"
        desc="Configure backend nodes, load balancing, discovery, and health checks."
      ...
      <AntdConfigProvider>
        ...
        <ProTable
          ...
          toolBarRender={() => [
            <SearchInput key="search" defaultValue={params.q ?? params.name ?? ''} placeholder="Search upstreams..." onSearch={(q) => setParams({ q, name: undefined, page: 1 })} />,
